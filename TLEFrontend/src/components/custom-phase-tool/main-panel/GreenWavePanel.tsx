...
const PhaseNavButton = styled.div`
  width: 32rem;
  height: 32rem;
  display: flex;
  align-items: center;
  ...
  font-weight: bold;
  transition: all 0.2s;
  &:hover { background: ${WERDER_GREEN_LIGHT}; transform: scale(1.05); }
`;
...
const RenameButton = styled.div<{$cancel?: boolean}>`
  padding: 10rem 16rem;
  border-radius: 6rem;
  cursor: pointer;
  font-weight: 600;
  background: ${props => props.$cancel ? 'rgba(255, 255, 255, 0.1)' : WERDER_GREEN};
  color: white;
  transition: all 0.2s;
  &:hover { background: ${props => props.$cancel ? 'rgba(255, 255, 255, 0.2)' : WERDER_GREEN_LIGHT}; }
`;
...
// === MOVE BUTTONS FOR INTERSECTION ORDERING ===
const DashboardMoveButtons = styled.div`
  display: flex;
  flex-direction: column;
  margin-right: 6rem;
`;

const DashboardMoveButton = styled.button<{$disabled?: boolean}>`
  background: ${props => props.$disabled ? 'rgba(255, 255, 255, 0.05)' : 'rgba(255, 255, 255, 0.1)'};
  border: none;
  color: ${props => props.$disabled ? 'rgba(255, 255, 255, 0.2)' : 'var(--textColor)'};
  padding: 2rem 6rem;
  border-radius: 3rem;
  ...
  &:hover {
    background: ${props => props.$disabled ? 'rgba(255, 255, 255, 0.05)' : WERDER_GREEN};
...
const DashboardRemoveButton = styled.button`
...
const DashboardProgressOverflow = styled.div<{$isGreen?: boolean; $visible: boolean}>`
  ...
  bottom: 0;
  background: ${p => p.$isGreen 
    ? `linear-gradient(90deg, ${WERDER_GREEN_LIGHT}80 0%, ${WERDER_GREEN_LIGHT}40 100%)`
...
export default function GreenWavePanel({ onClose }: GreenWavePanelProps) {
  ...
  const handlePhaseIndexChange = useCallback((delta: number) => {
    if (!syncSettings) return;
    const newIndex = Math.max(0, Math.min(syncSettings.phaseCount - 1, syncSettings.syncPhaseIndex + delta));
    call("C2VM.TLE", "CallUpdateSyncSettings", JSON.stringify({
      syncGroupId: syncSettings.syncGroupId, useSyncedCycle: syncSettings.useSyncedCycle,
      cycleOffsetSeconds: syncSettings.cycleOffsetSeconds, syncPhaseIndex: newIndex
    }));
  }, [syncSettings]);
  ...
  // === PHASE 2: INTERSECTION ORDERING HANDLERS ===
  const handleMoveIntersectionUp = useCallback((entityIndex: number) => {
    if (!currentGroup) return;
    call("C2VM.TLE", "CallMoveIntersectionUp", JSON.stringify({ 
      groupId: currentGroup.groupId, 
      entityIndex 
    }));
  }, [currentGroup]);
  
  const handleMoveIntersectionDown = useCallback((entityIndex: number) => {
    if (!currentGroup) return;
    call("C2VM.TLE", "CallMoveIntersectionDown", JSON.stringify({ 
      groupId: currentGroup.groupId, 
      entityIndex 
    }));
  }, [currentGroup]);
  
  const handleRemoveFromGroup = useCallback((entityIndex: number) => {
    call("C2VM.TLE", "CallRemoveIntersectionFromGroup", JSON.stringify({ entityIndex }));
    showToast(getString(locale, "GWP_IntersectionRemoved") || "Intersection removed from group", 'warning');
  }, [showToast]);
  
  // handleSetIntersectionOffset available for future use - currently offset is set via Kreuzung tab
  // const handleSetIntersectionOffset = useCallback((entityIndex: number, offsetRealSeconds: number) => {
  //   call("C2VM.TLE", "CallSetIntersectionOffset", JSON.stringify({ entityIndex, offsetRealSeconds }));
  // }, []);
  ...
  return (
    <PanelContainer>
      ...
      <Scrollable style={{ height: "auto", maxHeight: "75vh" }}>
        <ContentArea>
          ...
          {activeTab === 'intersection' && (
            <>
              ...
              <Section>
                ...
                {isRenaming && currentGroup && renamingGroupId === currentGroup.groupId && (
                  <RenameContainer style={{marginTop: '10rem'}}>
                    ...
                    <RenameButton onClick={handleConfirmRename}>✓</RenameButton>
                    <RenameButton $cancel onClick={() => { setIsRenaming(false); setRenamingGroupId(null); }}>X</RenameButton>
                  </RenameContainer>
                )}
              </Section>
              ...
              {syncSettings.syncGroupId > 0 && (
                <>
                  ...
                  <Section>
                    ...
                    <SettingRow>
                      <SettingIconWrapper><Tooltip position="right" tooltip={<TooltipText>{getString(locale, "GWP_TT_SyncPhase") || "Select which phase to synchronize. Phase #1 should be the main direction (Green Wave)."}</TooltipText>}><TooltipIcon /></Tooltip></SettingIconWrapper>
                      <SettingLabel>{getString(locale, "SyncPhase") || "Sync Phase"}</SettingLabel>
                      <SettingControl>
                        <PhaseNavButton onClick={() => handlePhaseIndexChange(-1)}>◀</PhaseNavButton>
                        <PhaseDisplay>Phase #{syncSettings.syncPhaseIndex + 1}</PhaseDisplay>
                        <PhaseNavButton onClick={() => handlePhaseIndexChange(1)}>▶</PhaseNavButton>
                      </SettingControl>
                    </SettingRow>
                    <SliderContainer>
              ...
              <Section>
                ...
                <HintBox><HintIcon>*</HintIcon>{getString(locale, "GWP_PhaseHint") || "Phase #1 should be the main direction for the Green Wave!"}</HintBox>
                <SaveFeedback>
                  <SaveIcon>✓</SaveIcon>
                        ...
                        <div style={{display: 'flex', alignItems: 'center', gap: '10rem', flex: 1}}>
                          ...
                          {isRenaming && renamingGroupId === group.groupId ? (
                            <div style={{display: 'flex', alignItems: 'center', gap: '6rem'}}>
                              ...
                              />
                              <span onClick={handleConfirmRename} style={{cursor: 'pointer', color: WERDER_GREEN_LIGHT}}>✓</span>
                              <span onClick={() => { setIsRenaming(false); setRenamingGroupId(null); }} style={{cursor: 'pointer', color: '#ff6b6b'}}>✕</span>
                            </div>
                          ) : (
                          ...
                          {!isRenaming && (
                            <div style={{display: 'flex', gap: '4rem'}}>
                              <span 
                                ...
                                style={{cursor: 'pointer', padding: '4rem 8rem', background: 'rgba(255,255,255,0.08)', borderRadius: '4rem', fontSize: '0.75em'}}
                              >
                                ✏️
                              </span>
                              {group.intersectionCount === 0 && (
                                <span 
                                  ...
                                  style={{cursor: 'pointer', padding: '4rem 8rem', background: 'rgba(200,50,50,0.2)', borderRadius: '4rem', fontSize: '0.75em', color: '#ff6b6b'}}
                                >
                                  🗑️
                                </span>
                              )}
                ...
                <>
                  ...
                  <DashboardGroupHeader>
                    ...
                    <DashboardGroupMeta>
                      ...
                      <DashboardMetaBadge>
                        <MetaIcon>⟳</MetaIcon>{currentGroup.baseCycleDuration}s
                      </DashboardMetaBadge>
                    </DashboardGroupMeta>
                  </DashboardGroupHeader>
                  ...
                  {currentGroup.intersections && currentGroup.intersections.map((intersection, idx) => (
                    <div key={idx}>
                      <DashboardIntersectionCard $isActive={intersection.isSyncPhaseActive}>
                        <DashboardCardTop>
                          {/* Move Buttons */}
                          <DashboardMoveButtons>
                            <DashboardMoveButton 
                              $disabled={idx === 0}
                              onClick={() => idx > 0 && handleMoveIntersectionUp(intersection.entity)}
                              title={getString(locale, "GWP_MoveUp") || "Move up"}
                            >
                              ▲
                            </DashboardMoveButton>
                            <DashboardMoveButton 
                              $disabled={idx === (currentGroup.intersections?.length || 1) - 1}
                              onClick={() => idx < (currentGroup.intersections?.length || 1) - 1 && handleMoveIntersectionDown(intersection.entity)}
                              title={getString(locale, "GWP_MoveDown") || "Move down"}
                            >
                              ▼
                            </DashboardMoveButton>
                          </DashboardMoveButtons>
                          ...
                          <DashboardCardTitle>
                            ...
                            <DashboardCardInfo>
                              <DashboardCardName>
                                {`Kreuzung #${intersection.index}`}
                                {idx === 0 && <DashboardReferenceTag>Referenz</DashboardReferenceTag>}
                              </DashboardCardName>
                              <DashboardCardSubtitle>{`${intersection.phaseCount} Phasen • ${intersection.cycleDuration}s`}</DashboardCardSubtitle>
                            </DashboardCardInfo>
                          </DashboardCardTitle>
                          <DashboardCardBadges>
                            <DashboardStatusBadge $active={intersection.isSyncPhaseActive} $type="phase">
                              {intersection.isSyncPhaseActive ? '● GRÜN' : `Phase ${intersection.currentPhase + 1}`}
                            </DashboardStatusBadge>
                            <DashboardStatusBadge $active={intersection.isSynced} $type="sync">
                              {intersection.isSynced ? 'SYNC' : 'AUS'}
                            </DashboardStatusBadge>
                          </DashboardCardBadges>
                        </DashboardCardTop>
                        ...
                        <DashboardProgressSection>
                          <DashboardProgressHeader>
                            <DashboardProgressLabel>
                              <Tooltip position="right" tooltip={<TooltipText>Links: Mindestdauer (Phase muss mindestens so lange laufen). Blinkend: Phase ist bereit zum Wechseln und wartet auf guenstigen Moment.</TooltipText>}>
                                <span style={{cursor: 'help'}}>{`Phase ${intersection.currentPhase + 1} von ${intersection.phaseCount}`}</span>
                              </Tooltip>
                            </DashboardProgressLabel>
                            <DashboardProgressValue $isGreen={intersection.isSyncPhaseActive}>
                              {intersection.isSyncPhaseActive 
                                ? (intersection.progress >= 50 ? 'Sync aktiv - wartend' : 'Grüne Welle aktiv!')
                                : (intersection.progress >= 50 ? 'Wechselbereit' : `${Math.round(intersection.progress * 2)}%`)}
                          ...
                          <DashboardProgressBarContainer>
                            ...
                            <DashboardProgressLabels>
                              ...
                              <DashboardProgressLabelCenter>
                                <span>50%</span>
                                <Tooltip position="bottom" tooltip={<TooltipText>Bei 50% ist die Mindestdauer erreicht. Die Phase kann ab hier jederzeit wechseln. Der blinkende Bereich zeigt, dass auf einen guenstigen Wechselzeitpunkt gewartet wird.</TooltipText>}>
                        ...
                        <DashboardStatsRow>
                          <DashboardCompactStat>
                            <DashboardCompactLabel>Offset</DashboardCompactLabel>
                          ...
                          <DashboardCompactStat>
                            <DashboardCompactLabel>Zyklus</DashboardCompactLabel>
                        ...
                        {(intersection.edgeCount !== undefined && intersection.edgeCount > 0) && (
                          <DashboardStatsRow>
                            <DashboardCompactStat>
                              <DashboardCompactLabel>Straßen</DashboardCompactLabel>
                              <DashboardSmallStatValue>{intersection.edgeCount}</DashboardSmallStatValue>
                            </DashboardCompactStat>
                            <DashboardCompactStat>
                              <DashboardCompactLabel>Spuren</DashboardCompactLabel>
                            ...
                            <DashboardCompactStat>
                              <DashboardCompactLabel>Fußg.</DashboardCompactLabel>
                        ...
                        <DashboardRemoveButton 
                          onClick={() => handleRemoveFromGroup(intersection.entity)}
                          title={getString(locale, "GWP_RemoveFromGroup") || "Remove from Group"}
                        >
                      ...
                      {idx < (currentGroup.intersections?.length || 0) - 1 && (
                        <DashboardOffsetConnector>
                          ...
                          <DashboardOffsetValue>
                            +{(currentGroup.intersections?.[idx + 1]?.offset || 0) - intersection.offset}s Versatz
                          </DashboardOffsetValue>
                          <DashboardOffsetLine />
                        </DashboardOffsetConnector>
                      )}
                  ...
                  <LiveSummary>
                    <LiveSummaryItem>
                      ...
                      <LiveSummaryLabel>Basis-Zyklus</LiveSummaryLabel>
                    </LiveSummaryItem>
                    <LiveSummaryDivider />
                    <LiveSummaryItem>
                      <LiveSummaryValue>
                      ...
                      <LiveSummaryLabel>Grüne Welle</LiveSummaryLabel>
                    </LiveSummaryItem>
                    <LiveSummaryDivider />
                    <LiveSummaryItem>
                      <LiveSummaryValue>
                      ...
                      <LiveSummaryLabel>Sync aktiv</LiveSummaryLabel>
                    </LiveSummaryItem>
                  </LiveSummary>
                  
                  <HintBox style={{marginTop: '16rem', backgroundColor: 'rgba(78, 184, 110, 0.1)'}}>
                    <HintIcon>i</HintIcon>
                    Nutze die Pfeiltasten um die Reihenfolge anzupassen. Die erste Kreuzung ist die Referenz (Offset 0).
                  </HintBox>
                </>
              )}
            </>
