...
function GetCustomPhaseLane(edge: EdgeInfo, index: number, type: CustomPhaseLaneType): CustomPhaseLane {
  ...
  if (type == "bicycleLane" && edge.m_EdgeGroupMask.m_Bicycle) {
    result.all = (edge.m_EdgeGroupMask.m_Bicycle.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.all;
  }
  return result;
}
...
export default function EdgePanel(props: {data: EdgeInfo, index: number, position: ScreenPoint}) {
  const clickHandler = useCallback((index: number, type: CustomPhaseLaneType, direction: CustomPhaseLaneDirection, currentSignal: CustomPhaseSignalState) => {
    ...
    if (type == "bicycleLane" && newGroupMask.m_Bicycle) {
      newSignal = currentSignal == "stop" ? "go" : "stop";
  ...
  // V134: Bicycle UI disabled - CS2 does NOT support separate bicycle signals!
  // =====================================================================
  // The game explicitly skips SecondaryLanes in traffic light processing
  // (PatchedTrafficLightInitializationSystem.cs line 419).
  // Bicycles automatically follow car signals - this is hardcoded in CS2.
  //
  // Backend (m_Bicycle in EdgeGroupMask) is kept for future compatibility
  // if Colossal Order ever adds bicycle signal support.
  //
  // To re-enable bicycle UI in future:
  // const bicycleLaneCount = props.data.m_BicycleLaneCount ?? 0;
  // =====================================================================
  const bicycleLaneCount = 0;
      ...
      <Content>
        <LaneContainer>
          ...
          {trackLaneCount > 0 && <>
            ...
            {bicycleLaneCount + props.data.m_PedestrianLaneStopLineCount + props.data.m_PedestrianLaneNonStopLineCount > 0 && <Divider />}
          </>}
          {/* V126: Bicycle support - single signal per approach */}
          {bicycleLaneCount > 0 && <>
            <Column>
