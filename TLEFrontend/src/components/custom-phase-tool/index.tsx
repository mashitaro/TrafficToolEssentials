...
export default function CustomPhaseTool() {
  ...
  return (
    <>
      {activeEditingCustomPhaseIndex >= 0 && edgeInfoList.filter(edge => (edge.m_EdgeGroupMask.m_Options & EdgeGroupMaskOptions.PerLaneSignal) == 0).map(edge => <EdgePanel key={edge.m_Position.key} data={edge} index={activeEditingCustomPhaseIndex} position={screenPointMap[edge.m_Position.key]} />)}
      {activeEditingCustomPhaseIndex >= 0 && edgeInfoList.filter(edge => (edge.m_EdgeGroupMask.m_Options & EdgeGroupMaskOptions.PerLaneSignal) != 0).map(edge => edge.m_SubLaneInfoList.map(subLane => <SubLanePanel key={subLane.m_Position.key} edge={edge} subLane={subLane} index={activeEditingCustomPhaseIndex} position={screenPointMap[subLane.m_Position.key]} />).flat())}
    </>
  );
}