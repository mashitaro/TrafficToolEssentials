...
namespace C2VM.TrafficToolEssentials.Systems.Tool;
...
public partial class ToolSystem : NetToolSystem
{
    ...
    // V231: Cached EntityQuery to prevent memory leak
    ...
    {
        Mod.LogDebug($"Searching for traffic light prefab asset entities");
        ...
        EntityQuery query = m_PlaceableNetDataQuery;
        
        // V218: SAFE try-finally pattern
        NativeArray<Entity> entityArray = default;
        NativeArray<PlaceableNetData> placeableNetDataArray = default;
        try
        {
        entityArray = query.ToEntityArray(Allocator.Temp);
        placeableNetDataArray = query.ToComponentDataArray<PlaceableNetData>(Allocator.Temp);
        for (int i = 0; i < entityArray.Length; i++)
        {
            ...
            if (m_PrefabSystem.TryGetPrefab(entityArray[i], out PrefabBase prefabBase) && prefabBase is NetPrefab)
            {
                ...
                Mod.LogDebug($"{m_PrefabEntity} prefabBase.uiTag: {prefabBase.uiTag}");
            }
        }
        ...
        finally
        {
            if (entityArray.IsCreated)
                entityArray.Dispose();
            if (placeableNetDataArray.IsCreated)
                placeableNetDataArray.Dispose();
        }
