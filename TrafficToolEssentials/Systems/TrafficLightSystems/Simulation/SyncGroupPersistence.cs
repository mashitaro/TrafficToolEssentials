...
public static class SyncGroupPersistence
{
    ...
    public static bool SaveGroups(List<SyncGroupData> groups, uint nextGroupId, string cityName = "default")
    {
        try
        {
            ...
            Mod.m_Log.Info($"[SyncGroupPersistence] Saved {groups.Count} groups to {filePath}");
            return true;
        }
        catch (Exception ex)
        {
            Mod.m_Log.Error($"[SyncGroupPersistence] Failed to save groups: {ex.Message}");
            return false;
        }
    }
    ...
    public static SyncGroupsFile LoadGroups(string cityName = "default")
    {
        try
        {
            ...
            {
                Mod.m_Log.Info($"[SyncGroupPersistence] No saved groups found at {filePath}");
            ...
            Mod.m_Log.Info($"[SyncGroupPersistence] Loaded {data.Groups?.Count ?? 0} groups from {filePath}");
            return data;
        }
        catch (Exception ex)
        {
            Mod.m_Log.Error($"[SyncGroupPersistence] Failed to load groups: {ex.Message}");
            return new SyncGroupsFile();
        }
    }
    ...
    public static bool DeleteGroupsFile(string cityName = "default")
    {
        try
        {
            ...
            if (File.Exists(filePath))
            {
                ...
                Mod.m_Log.Info($"[SyncGroupPersistence] Deleted groups file: {filePath}");
            }
            return true;
        }
        catch (Exception ex)
        {
            Mod.m_Log.Error($"[SyncGroupPersistence] Failed to delete groups file: {ex.Message}");
            return false;
        }
    }
}
