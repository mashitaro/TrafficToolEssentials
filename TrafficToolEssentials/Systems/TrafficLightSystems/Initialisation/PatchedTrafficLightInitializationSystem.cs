...
public partial class PatchedTrafficLightInitializationSystem : Game.GameSystemBase
{
    ...
    // BitDefender false positive is acceptable - all major AV scanners report clean
    [BurstCompile]
    public struct InitializeTrafficLightsJob : IJobChunk
    {
        ...
        {
            // V218: SAFE try-finally pattern for all NativeList/NativeHashMap allocations
            NativeList<LaneGroup> groups = default;
            NativeList<LaneGroup> vehicleLanes = default;
            NativeList<LaneGroup> pedestrianLanes = default;
            ...
            groups = new NativeList<LaneGroup>(16, Allocator.TempJob);
            vehicleLanes = new NativeList<LaneGroup>(16, Allocator.TempJob);
            pedestrianLanes = new NativeList<LaneGroup>(16, Allocator.TempJob);
            ...
            for (int i = 0; i < nativeArray3.Length; i++)
            {
                ...
                {
                    // V284: CRITICAL FIX - Use TempJob for parallel jobs
                    var edgeInfoList = NodeUtils.GetEdgeInfoList(Allocator.TempJob, entityArray[i], ref this, subLanes, connectedEdgeAccessor[i], edgeGroupMaskAccessor[i], subLaneGroupMaskAccessor[i]);
                    var edgeInfoArray = edgeInfoList.AsArray();
                    ...
                    // V200: Properly dispose the NativeList (which also disposes inner SubLaneInfoLists)
            ...
            // V218: Dispose moved to finally block for exception safety
            
            } // end try
            finally
            {
                // V218: Ensure cleanup even on exception
                if (groups.IsCreated)
                    groups.Dispose();
                if (vehicleLanes.IsCreated)
                    vehicleLanes.Dispose();
                if (pedestrianLanes.IsCreated)
        ...
        private void ProcessMoveableBridgeLanes(Entity entity, NativeList<LaneGroup> vehicleLanes, NativeList<LaneGroup> pedestrianLanes, NativeList<LaneGroup> groups, DynamicBuffer<SubLane> subLanes, MoveableBridgeData moveableBridgeData, bool isSubNode, ref NativeHashMap<PathNode, int> groupIndexMap, out int groupCount)
        {
            ...
            {
                // V284: CRITICAL FIX - Use TempJob for parallel jobs
                groupIndexMap = new NativeHashMap<PathNode, int>(32, Allocator.TempJob);
            }
