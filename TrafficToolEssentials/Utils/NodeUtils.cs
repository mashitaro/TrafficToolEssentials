...
namespace C2VM.TrafficToolEssentials.Utils;
...
public partial struct NodeUtils
{
    ...
    (
        ...
        // V230: New parameter to fix memory leak in managed code!
        // When true: Copy SubLaneInfoList to new NativeArray and dispose NativeList
        // When false (default): Original behavior for Burst jobs (auto-disposed)
        bool copyAndDisposeSubLaneList = false
    )
    {
        ...
        foreach (ConnectedEdge connectedEdge in connectedEdgeBuffer)
        {
            ...
            foreach (SubLane nodeSubLane in nodeSubLaneBuffer)
            {
                ...
                if (laneConnection.m_SourceEdge == edgeEntity)
                {
                    if (!masterLaneLookup.HasComponent(nodeSubLane.m_SubLane))
                    {
                        ...
                        // V134: SecondaryLane (Bicycle Lane) detection
                        // =====================================================================
                        // NOTE: CS2 does NOT support separate bicycle traffic signals!
                        // The game explicitly skips SecondaryLanes in traffic light processing
                        // (see PatchedTrafficLightInitializationSystem.cs line 419).
                        // Bicycles automatically follow the car signals on the same road.
                        //
                        // This detection code is kept for:
                        // 1. UI display (showing bicycle lanes exist on this approach)
                        // 2. Future compatibility if Colossal Order adds bicycle signal support
                        //
                        // The m_Bicycle field in EdgeGroupMask is already prepared for this.
                        // =====================================================================
                        else if (secondaryLaneLookup.HasComponent(nodeSubLane.m_SubLane))
                        {
            ...
            // V230: Fix memory leak for managed code!
            // AsArray() only returns a reference - NativeList metadata (~24 bytes) is never freed!
            // For managed code: Copy to new NativeArray and dispose NativeList
            // For Burst jobs: Keep original behavior (Burst auto-disposes Allocator.Temp)
            if (copyAndDisposeSubLaneList)
            {
                edgeInfo.m_SubLaneInfoList = new NativeArray<SubLaneInfo>(subLaneInfoList.AsArray(), allocator);
            ...
            // V200: Dispose subLaneMap - it's Allocator.Temp and no longer needed
            subLaneMap.Dispose();
        }

        // V200: Dispose laneConnectionMap - it's Allocator.Temp and no longer needed
        laneConnectionMap.Dispose();
        return edgeInfoList;
    }

    // V230: This overload is ONLY called from managed code (UISystem)
    // So we can safely use copyAndDisposeSubLaneList=true to fix memory leaks!
    public static NativeList<EdgeInfo> GetEdgeInfoList(Allocator allocator, Entity nodeEntity, Systems.UI.UISystem uISystem)
    {
        ...
        (
            ...
            copyAndDisposeSubLaneList: true  // V230: Fix leak in managed code!
        );
    }

    // V230: This overload is called from Burst jobs (PatchedTrafficLightInitializationSystem)
    // Burst auto-disposes Allocator.Temp, so we use copyAndDisposeSubLaneList=false (default)
    // DO NOT change this - manual dispose in Burst = crash!
    ...
    // V200: Fixed Dispose method - works with NativeList (the actual return type of GetEdgeInfoList)
    public static void Dispose(NativeList<EdgeInfo> edgeInfoList)
    {
        for (int i = 0; i < edgeInfoList.Length; i++)
        {
            edgeInfoList[i].m_SubLaneInfoList.Dispose();
        }
        edgeInfoList.Dispose();
    }
    
    // V200: Keep array version for backwards compatibility, but mark as internal
    public static void Dispose(NativeArray<EdgeInfo> edgeInfoArray)
    {
        foreach (var edgeInfo in edgeInfoArray)
        {
            edgeInfo.m_SubLaneInfoList.Dispose();
        }
        // Note: Do NOT dispose the array itself if it came from .AsArray() - the NativeList owns it!
        // Only dispose if you allocated the array yourself
    }
