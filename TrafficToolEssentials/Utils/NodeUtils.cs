            ...
            foreach (SubLane nodeSubLane in nodeSubLaneBuffer)
            {
                ...
                if (laneConnection.m_SourceEdge == edgeEntity)
                {
                    if (!masterLaneLookup.HasComponent(nodeSubLane.m_SubLane))
                    {
                        ...
                        // V126: Pure SecondaryLane (Bicycle Lane) WITHOUT CarLane component
                        // Bicycles in CS2 don't have direction-specific routing at intersections
                        // When green, cyclists can travel to ANY connected bicycle path
                        // Therefore we only need a simple count per approach, not Left/Straight/Right
                        else if (secondaryLaneLookup.HasComponent(nodeSubLane.m_SubLane))
                        {
                            ...
                            subLaneMap[laneConnection.m_SourceSubLane] = sourceSubLaneInfo;
                            UnityEngine.Debug.Log($"[V126-BICYCLE] Found bicycle lane! Edge {edgeEntity.Index}, Total count: {edgeInfo.m_BicycleLaneCount}");
                        }
                    }
                }
